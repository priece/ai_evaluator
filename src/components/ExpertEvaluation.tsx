...
export default function ExpertEvaluation() {
  ...
  const [evaluations, setEvaluations] = useState<ExpertEvaluationType[]>([]);
  const [isModalOpen, setIsModalOpen] = useState<boolean>(false);
  const [newSessionName, setNewSessionName] = useState<string>('');
  const [expertScoreInput, setExpertScoreInput] = useState<string>('');
  ...
  const fetchSessionEvaluations = async (sessionId: string) => {
    try {
      ...
      const data = await res.json();
      if (data.success) {
        const expertEvaluations = data.expertEvaluations || [];
        setEvaluations(expertEvaluations);
        // 根据已有评估记录计算当前轮次
        if (expertEvaluations.length > 0) {
          const maxRound = Math.max(...expertEvaluations.map((e: ExpertEvaluationType) => e.round));
  ...
  const submitEvaluation = async () => {
    ...
    const expertScore = parseInt(expertScoreInput, 10);
    if (isNaN(expertScore) || expertScore < 0 || expertScore > 100) {
      alert('请输入有效的专家评分（0-100）');
      return;
    }
    try {
      const res = await fetch('/api/expert-evaluation', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ sessionId: session.session_id, round: currentRound, expertScore }),
      });
      const data = await res.json();
      if (data.success) {
        setLatestScore(data.evaluation.expert_score);
        setExpertScoreInput('');
    ...
    <div className="p-6 h-full">
      <div className="flex gap-6 h-full">
        ...
        <div className="w-1/2 bg-white rounded-lg shadow p-6 flex flex-col">
          ...
          {/* 专家评分输入 */}
          <div className="mb-4">
            <label className="block text-sm font-medium mb-2">专家评分 (0-100)</label>
            <input
              type="number"
              min="0"
              max="100"
              value={expertScoreInput}
              onChange={(e) => setExpertScoreInput(e.target.value)}
              placeholder="请输入评分"
              className="w-full border rounded-lg px-3 py-2"
              disabled={!session}
            />
          ...
          <button
            ...
            disabled={!session || !expertScoreInput}
